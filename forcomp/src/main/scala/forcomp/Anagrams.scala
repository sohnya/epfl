...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (for {
      (a, b) <- w.toLowerCase() groupBy ((element: Char) => element)
    } yield (a, b.length)).toList sortBy (_._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case List() => List(List())
    case x :: xs => for {
      current <- (0 to x._2).map((x._1, _)).toList
      rest <- combinations(xs)
    } yield if (current._2 == 0) rest else current :: rest
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val b = y.toMap withDefaultValue 0
    (x.toMap map (x => (x._1, x._2 - b(x._1)))).filter(_._2 != 0).toList.sorted
